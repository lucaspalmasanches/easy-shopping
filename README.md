# Easy Shopping 🛍️

![Status](https://img.shields.io/badge/Status-Conclu%C3%ADdo-brightgreen)

## Sobre o Projeto ✨

Este é um projeto desenvolvido como parte do meu aprendizado no [DevClub](https://www.devclub.com.br/). O "Easy Shopping" é uma **interface de e-commerce responsiva**, focada em demonstrar habilidades de construção de layout moderno e adaptável para diferentes dispositivos, utilizando HTML e CSS.

Com este projeto, busco aplicar as melhores práticas de desenvolvimento front-end e solidificar meus conhecimentos em estruturação e estilização web.

---

## 💻 Tecnologias Utilizadas

Aqui estão as tecnologias que utilizei para construir este projeto:

![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)

---

## ✨ Funcionalidades

*   **Layout Responsivo:** Adapta-se perfeitamente a telas de desktop e dispositivos móveis.

*   **Interface Intuitiva:** Design pensado para uma navegação fácil e agradável.

*   **Design Moderno:** Estilização com foco em uma experiência de usuário atrativa.

*   **Estrutura Semântica:** Uso correto de tags HTML para melhor acessibilidade e SEO.

---

## 📸 Visualização do Projeto

Confira o projeto rodando em diferentes dispositivos:

<p align="center">
  <img src="https://github.com/lucaspalmasanches/easy-shopping/blob/main/assets/EasyShoppingPC.png?raw=true" alt="Easy Shopping - Versão Desktop" width="48%">
  <img src="https://github.com/lucaspalmasanches/easy-shopping/blob/main/assets/EasyShoppingMobile.png?raw=true" alt="Easy Shopping - Versão Mobile" width="48%">
</p>

---

## 📚 Aprendizados

Este projeto foi uma experiência valiosa para consolidar meus conhecimentos em:

*   **HTML Semântico:** Criação de estruturas web claras e acessíveis.

*   **CSS Avançado:** Utilização de Flexbox para layouts, media queries para responsividade e variáveis CSS.

*   **Controle de Versão:** Gerenciamento de código com Git e GitHub, incluindo commits, branches e pushes.

---

## Conecte-se Comigo 🤝

Estou sempre aberto a novas conexões e oportunidades. Sinta-se à vontade para entrar em contato!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lucas-palma-sanches-082902426)
[![E-mail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lucaspalma331@gmail.com)
